      ...
      <form class="mt-8 space-y-6" @submit="register">
        ...
        <div class="-space-y-px rounded-md shadow-sm">
          ...
          <div>
            ...
            <input id="password_confirm" name="password_confirm" type="password" autocomplete="current-password_confirm" required="" v-model="user.password_confirmation" class="relative block w-full appearance-none rounded-none  border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="Confirm Password" />
          </div>
        </div>
        <div>
...
function register(ev) {
  ...
  //register should return a promise so
  .then((res) => {
    //navigate user somwhere
    router.push({
      name: 'Dashboard'
    })
  })
